...
- Модалка карточки с голосованием «за» и «против». Повторный клик отменяет голос, за свою
  карточку голосовать нельзя.
- Создание карточки: заголовок, тип, описание, картинка по ссылке, дата.
- Страница карточки: подробности, редактирование и удаление своей карточки, комментарии
...
- `useEffect`: `shared/ui/Modal` подписывается на Escape и отписывается в cleanup.
  `pages/card` загружает данные и выбрасывает устаревший ответ через флаг `ignore`.
- Иммутабельность: `setCard((prev) => prev && { ...prev, ...changed })` в `pages/card`,
  `replaceCard` в `entities/card/model/cards-store.ts`.
- Модалка: `shared/ui/Modal` и `widgets/card-modal`.
...
- Страница карточки открывается из модалки. Ещё появляются страница пользователя и компонент новой карточки.
